...
const ProductDetails = () => {
    ...
    const [serverReview, setServerReview] = useState([]);
    ...
    //get product review from firebase
    const getReview = onSnapshot(collection(db, "reviews"), (snapShot) => {
        let list = [];
        snapShot.docs.forEach((doc) => {
            list.push({ id: doc.id, ...doc.data() });
        });

        //for every review in list if productId === id then push it to serverReview
        list.forEach((item) => {
            if (item.productId === id) {
                serverReview.push(item);
            }
        });

        setServerReview(serverReview);
        console.log("Server Review is :", serverReview);
    });
